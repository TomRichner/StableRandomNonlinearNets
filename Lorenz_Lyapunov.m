...
X_k = [-4.7795941536989090537     -8.8325236589201416848      10.425932436960884786]'; % X_0

% Initial small perturbation
d0 = 1e-4; % Initial separation magnitude
perturbation_vec = randn(3,1); % Random initial direction for perturbation
perturbation_vec = (perturbation_vec / norm(perturbation_vec)) * d0;
X_k_pert = X_k + perturbation_vec;

% Time parameters for saving and plotting the state
fs = 100; % plotting sample frequency
dt = 1/fs; % plotting sample time
T = [-100, 1000]; % plotting time interval, from T(1) to 0, is warmup and settle down which isn't typically plotted.
if not( T(1)<=0 & 0<T(2) )
...
%% --- 4. System Integration (single call) ---
% Integrate the fiducial trajectory once on the entire time grid `t`.
[t_ode, X_int] = ode45(@(tt,XX) lorenz_eqs(tt,XX,params), t, X_k, ode_options);
assert(all(abs(t_ode - t) < 1e-13), 'ODE solver did not return results exactly at the requested times.');

%% --- 5. Benettin's algorithm on a decimated grid -----------------         
deci_lya = round(0.1*fs);     % samples per Lyapunov interval
...
for k = 1:nt_lya
    ...
    X_start = X_int(idx_start,:).';      % fiducial state at t_lya(k)
    ...
    [~, X_pert_seg] = ode45(@(tt,XX) lorenz_eqs(tt,XX,params), t_seg, X_k_pert, ode_options);
    
    X_pert_end = X_pert_seg(end,:).';

    X_end   = X_int(idx_end,:).';        % fiducial state tau_lya later
