...
% u_ex = u_ex+0.1./fs.*randn(n,nt); % a tiny bit of noise to help the network get off the trivial saddle node from ICs
...
if n_b == 1
    tau_b = 1*tau_STD; % if tau_STD << tau_b, then the system is very stable and close to edge of chaos
end
...
if use_Jacobian 
    ...
    ode_options = odeset('RelTol', 1e-7, 'AbsTol', 1e-8, 'MaxStep',0.5*dt, 'InitialStep', 0.1*dt); % fast
end
