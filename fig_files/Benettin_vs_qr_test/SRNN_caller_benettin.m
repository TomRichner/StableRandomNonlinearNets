...
if use_Jacobian 
    ...
    ode_options = odeset('RelTol', 1e-8, 'AbsTol', 1e-9, 'MaxStep', dt, 'InitialStep', 0.05*dt, 'Jacobian', SRNN_Jacobian_wrapper); % fast
else
    % ode_options = odeset('RelTol', 1e-5, 'AbsTol', 1e-6, 'MinStep', 0.1*dt,'MaxStep', dt, 'InitialStep', 0.5*dt); % fast
    ode_options = odeset('RelTol', 1e-8, 'AbsTol', 1e-9, 'MaxStep',dt, 'InitialStep', 0.05*dt); % RelTol must be less than perturbation d0, which is 1e-3
end
...
save_some_figs_to_folder_2('fig_files/Benettin_vs_qr_test/figs',['Benettin_seed_' num2str(seed)],[],[])
save('fig_files/Benettin_vs_qr_test/figs/Benettin_works_space.mat')