...
%% Analysis Parameters
n_levels = 3; % 8 Number of values to test for each parameter in the grid
note = 'param_space_n_n_b_E_1_short'
...
for c_idx = 1:length(conditions)
    ...
    batch_size = 100; % Number of parameter configs per batch
    ...
    total_elapsed_time = toc(overall_start_time);
    ...
    for c_idx = 1:num_conditions
        ...
        save_filename = fullfile(output_dir, sprintf('param_space_results_%s.mat', condition_name));
        if exist(save_filename, 'file'), delete(save_filename); end
        
        matObj.(condition_name) = matfile(save_filename, 'Writable', true);
        matObj.(condition_name).results = cell(num_combinations, 1);
    end
        ...
        if exist(batch_save_filename, 'file')
            ...
            if isfield(loaded_data, 'batch_results_by_condition')
                for c_idx = 1:num_conditions
                    ...
                    matObj.(condition_name).results(start_idx:end_idx, 1) = batch_slice;
                end
            else
    ...
    for c_idx = 1:num_conditions
        ...
        metadata.condition = conditions{c_idx};
        
        matObj.(condition_name).metadata = metadata;
        
        summary = struct();
        summary.metadata = metadata;
        summary.save_filename = matObj.(condition_name).Properties.Source;
        all_conditions_summary.(condition_name) = summary;
    end
