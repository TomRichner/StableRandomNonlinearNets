function fig_handle = tau_a_sensitivity_plot(param_file, hist_edges_with_inf, variable_to_plot, y_axis_label, custom_y_ticks, custom_x_ticks)
    ...
    %   custom_y_ticks       - (Optional) vector of user-defined y-ticks
    %   custom_x_ticks       - (Optional) vector of user-defined x-ticks
    %
    % Outputs:
    ...
    if exist('step_size', 'var')
        ...
        if nargin < 5 || isempty(custom_y_ticks)
            % If no custom ticks are provided, use the default logic
            ...
            % Use user-provided ticks
            intermediate_ticks = custom_y_ticks;
        end
    ...
    % Handle custom x-ticks
    if nargin >= 6 && ~isempty(custom_x_ticks)
        xticks(custom_x_ticks);
    else
