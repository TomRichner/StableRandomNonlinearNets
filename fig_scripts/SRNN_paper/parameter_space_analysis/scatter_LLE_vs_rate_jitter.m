...
for i = 1:num_conditions
    ...
    jitter_std = 0.035;
    lles_jittered = lles + jitter_std * randn(size(lles));
    mean_rates_jittered = mean_rates + jitter_std * randn(size(mean_rates));

    plot_handles(i) = scatter(lles_jittered, mean_rates_jittered, 15, color, 'filled', 'MarkerFaceAlpha', 0.35);
...
%% Save the figure
output_dir_for_figs = fullfile(base_dir, 'analysis_plots_jitter');

fprintf('\nSaving figure to: %s\n', output_dir_for_figs);
% Assuming save_some_figs_to_folder_2 is on the path
% If not, you might need to add its location to the path.
save_some_figs_to_folder_2(output_dir_for_figs, 'LLE_vs_rate_scatter_jitter', fig_scatter.Number, {'fig', 'svg', 'png'});
