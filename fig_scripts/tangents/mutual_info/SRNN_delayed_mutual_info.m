...
% sigma = 1;
% n_in = 3;
% u_ex(1:n_in,:) = mu + sigma.*randn(n_in,nt);

n_in = 3;
ch_in = 1:n_in; % input channels
sigma = 1; % standard deviation of gaussian random noise
dur = 2.5; % duration in seconds of each level
...
% --- extract last 1 second from each epoch ---
sec_from_end = 1; % seconds from end of each epoch
samples_from_end = sec_from_end*fs;

mu_end_index; 
n_epochs = length(mu_end_index);
...
for i_epoch = 1:n_epochs
    mu_this_epoch = mu_t(mu_end_index(i_epoch)); % the stimulus level for this epoch
...
for i_level = 1:n_levels
    ss_data_1D{1,i_level} = mean(ss_data{1,i_level},1)'; % collapse from multiple channels to 1 and make it a column
end

ss_data_1D{4} = [ss_data_1D{1}; ss_data_1D{2}; ss_data_1D{3}]; % fourth column is a combo of the three levels

for i = 1:length(ss_data_1D)-1
    PxSigma(i) = length(ss_data_1D{i})/length(ss_data_1D{length(ss_data_1D)});
end

% --- get probability dist
step = 0.01;
mn = min(ss_data_1D{4})-step;
mx = max(ss_data_1D{4})+step;
% dist = 'log10';
dist = 'linear';
[px, nx, bins] = isi_to_px(ss_data_1D,mn,step,mx,dist,1);
px = px(2:end,:) % remove the -inf bin
nx = nx(2:end,:);
bins = bins(2:end);
figure(3);
plot(bins,px(:,1:3))
