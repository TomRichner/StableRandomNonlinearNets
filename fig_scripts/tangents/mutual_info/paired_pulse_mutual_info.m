...
pWidth2 = 5;       % seconds, width of the second pulse
...
Amp1Levels = 5;
pAmp1 = randi([1, Amp1Levels], 1, nPairs);
...
[MI] = mutual_info_SISO(data_in, data_out, n_bins_in, n_bins_out)

%% Delayed mutual information analysis
delay_vec = 1:fix(fs*pWidth2); % delays in samples from pulse 2 start
...
for i_delay = 1:length(delay_vec)
    ...
    if sum(valid_pairs) > 1 % Need at least 2 data points for MI
        ...
        % data_out_delayed = data_out_delayed + (10/fs).*randn(size(data_out_delayed)); % measurement noise
...
%% Plot MI vs delay
figure
plot(delay_vec/fs, MI_vs_delay, 'LineWidth', 2)
xlabel('Delay from pulse 2 start (s)')
ylabel('Mutual Information (bits)')
title('Mutual Information vs Readout Delay during Pulse 2')
grid on
xlim([0, pWidth2])
