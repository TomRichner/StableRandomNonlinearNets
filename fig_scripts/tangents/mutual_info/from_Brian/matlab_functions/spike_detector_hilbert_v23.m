...
%     if nargout>5; envelope_pdf(ist+(i>1)*(3*wR):isp-(i<length(index_stop))*(3*wR),:)=sub_envelope_pdf(1+(i>1)*(3*wR):end-(i<length(index_stop))*(3*wR),:); end
    ...
    size_sub_d=(size(sub_d,1)-(i<length(index_stop))*(3*wR))-(1+(i>1)*(3*wR))+1; % po decimaci mùže zmizet vzorek a nelze pøesnì indexovat
    if nargout>2; d_decim(ist+(i>1)*(3*wR):ist+(i>1)*(3*wR)+size_sub_d-1,:)=sub_d(1+(i>1)*(3*wR):end-(i<length(index_stop))*(3*wR),:); end
    if nargout>3; envelope(ist+(i>1)*(3*wR):ist+(i>1)*(3*wR)+size_sub_d-1,:)=sub_envelope(1+(i>1)*(3*wR):end-(i<length(index_stop))*(3*wR),:); end
    if nargout>4; background(ist+(i>1)*(3*wR):ist+(i>1)*(3*wR)+size_sub_d-1,:,:)=sub_background(1+(i>1)*(3*wR):end-(i<length(index_stop))*(3*wR),:,:); end
    if nargout>5; envelope_pdf(ist+(i>1)*(3*wR):ist+(i>1)*(3*wR)+size_sub_d-1,:)=sub_envelope_pdf(1+(i>1)*(3*wR):end-(i<length(index_stop))*(3*wR),:); end
