...
%% adaptation 
sfa_std_both_none = 'sfa'; % 'none', 'both', 'sfa', 'std'
...
% params.activation_function = @(x) max(min(x+1.*x.^3, 30),0);   % added cubic nonlinearity
% params.activation_function = @(x) max(min(x,1),0);   % relu with saturation at 1
...
%% print integration performance stats
sim_dur = toc
