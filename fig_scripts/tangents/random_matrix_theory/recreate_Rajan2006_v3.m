...
mean_in_out_degree = n; % desired mean number of connections in and out
...
A_E = A(:,isE);
A_I = A(:,not(isE));
actual_variance_E = var(A_E(:))
actual_variance_I = var(A_I(:))
actual_alpha = actual_variance_I/actual_variance_E
b=b
...
disk_radius = sqrt(1-f+f/actual_alpha)

corrected_A = A-disk_radius*eye(size(A));
corrected_var_A = var(corrected_A(:))
corrected_stdev_A = std(corrected_A(:))
corrected_mean_A = mean(corrected_A(:))

corrected_A_E = corrected_A(:,isE);
corrected_A_I = corrected_A(:,not(isE));
corrected_variance_E = var(corrected_A_E(:))
corrected_variance_I = var(corrected_A_I(:))
corrected_alpha = corrected_variance_I/corrected_variance_E
b=b

eig_A = eig(corrected_A);

%%
disk_radius_corrected = sqrt(1-f+f/corrected_alpha)

disk_radius_difference = disk_radius_actual - disk_radius_corrected

disk_radius = disk_radius_corrected
...
hold on
plot(disk_radius*sin(t)-disk_radius,disk_radius*cos(t),'r','LineWidth',2)
hold on
...
Expected_LLE_if_zero_mean = disk_radius-disk_radius
...
figure(3)
set(gcf,'Position',[-564    37   560   420])
imagesc(A,[-2 2])
colorbar
