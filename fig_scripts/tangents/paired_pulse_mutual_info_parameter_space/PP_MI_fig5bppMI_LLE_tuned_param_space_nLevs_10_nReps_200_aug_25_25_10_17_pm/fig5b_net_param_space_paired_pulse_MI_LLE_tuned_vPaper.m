% fig5b_network_parameter_space_analysis_paired_pulse_MI_LLE_tuned_vPaper.m
% Explore SRNN parameter space with paired-pulse stimulus and compute MI vs delay.
% Uses SRNN_caller_wrapped_for_paired_pulse_MI_LLE_tuned to tune M to target LLE.
% Only proceeds to full simulation when Phase-1 numerical LLE is in [0.05, 1].
...
%% Parameters for Grid Search (sweep only target_LLE here)
params_for_grid = {'target_LLE'};

%% Analysis Parameters
 n_levels = 10; % Number of values to test for each parameter in the grid
 n_repetitions = 200; % Number of repetitions (different seeds) per parameter combination
 note = 'ppMI_LLE_tuned_param_space';
...
output_dir_base = fullfile(pwd, ['PP_MI_fig5b' note '_nLevs_' num2str(n_levels) '_nReps_' num2str(n_repetitions) '_' dt_str]);
if ~exist(output_dir_base, 'dir')
    mkdir(output_dir_base);
end

%% copy current mfile and wrapper into it
copyfile([mfilename('fullpath') '.m'], output_dir_base);
copyfile(fullfile(fileparts(mfilename('fullpath')), 'SRNN_caller_wrapped_for_paired_pulse_MI_LLE_tuned.m'), output_dir_base);
...
fprintf('\n\n======================================================\n');
fprintf('===== Running Paired-Pulse MI LLE-Tuned Analysis for All Conditions =====\n');
fprintf('======================================================\n\n');
...
p_default.n_a_E = 1; % default per condition override
p_default.n_b_E = 0; % default per condition override
p_default.target_LLE = 0.3; % default target
...
fprintf('=== Paired-Pulse LLE-Tuned Parameter Space Exploration Started for All Conditions ===\n');
...
for batch_idx = 1:num_batches
    ...
    % for j = 1:num_jobs_in_batch
        ...
        % try
            result_sim = SRNN_caller_wrapped_for_paired_pulse_MI_LLE_tuned(...
                sim_seed, current_p.target_LLE, current_p.n, current_p.EE_factor, current_p.IE_factor, ...
...
fprintf('=== OVERALL LLE-TUNED PARAMETER SPACE EXPLORATION COMPLETE ===\n');
