% plot_parameter_space_results_paired_pulse_MI_multi_hardcoded.m
%
% A version of plot_parameter_space_results_paired_pulse_MI_multi.m
% that uses hard-coded dataset paths instead of a UI selection.
%
...
%% --- Hard-coded dataset configuration ---
% Define datasets to be merged here.
% Each entry in the datasets struct should have:
% .base_dir: The full path to the experiment folder.
% .lle_range: A two-element vector [LLE_min, LLE_max] for filtering.
%
% Example:
% datasets = struct(...
%     'base_dir', ...
...
datasets = struct(...
    'base_dir', ...
        {fullfile(script_dir, 'PP_MI_fig5bppMI_param_space_nLevs_5_nReps_4_aug_22_25_ 5_21_pm'), ...
         fullfile(script_dir, 'PP_MI_fig5bppMI_LLE_tuned_param_space_nLevs_10_nReps_200_aug_25_25_10_17_pm')}, ...
    'lle_range', ...
        {[-10, 1], ...    % LLE range for dataset 1
         [0, 1]} ...   % LLE range for dataset 2
);
% You can add more datasets by extending the cell arrays above.

% --- End of hard-coded configuration ---

% --- Data loading and validation ---
num_datasets = numel(datasets);
fprintf('Found %d hard-coded datasets to process.\n', num_datasets);

for di = 1:num_datasets
    % Validate LLE range
    if ~isnumeric(datasets(di).lle_range) || numel(datasets(di).lle_range) ~= 2
        error('Invalid LLE range for dataset %d. It must be a two-element numeric vector.', di);
    end
    datasets(di).lle_range = sort(datasets(di).lle_range(:))';
    
    % Print info
    lr_vals = datasets(di).lle_range;
    fprintf('Dataset %d: %s with LLE range [%g, %g]\n', di, datasets(di).base_dir, lr_vals(1), lr_vals(2));
    
    % Check if directory exists
    if ~exist(datasets(di).base_dir, 'dir')
        warning('Directory not found for dataset %d: %s', di, datasets(di).base_dir);
    end
end
