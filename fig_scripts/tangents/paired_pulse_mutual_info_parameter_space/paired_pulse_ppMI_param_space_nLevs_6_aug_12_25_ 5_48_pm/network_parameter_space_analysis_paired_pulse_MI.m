...
%% Analysis Parameters
n_levels = 6; % Number of values to test for each parameter in the grid
...
for c_idx = 1:length(conditions)
    ...
    fprintf('\n\n======================================================\n');
    fprintf('===== Running Paired-Pulse MI Analysis for: %s =====\n', upper(current_condition.name));
    ...
    fprintf('=== Paired-Pulse Parameter Space Exploration Started for %s ===\n', current_condition.name);
    ...
    % Store summary for this condition (for backward compatibility)
    summary = struct(); %#ok<NASGU>
    summary.metadata = metadata;
    summary.save_filename = matObj.(condition_name2).Properties.Source; %#ok<NASGU>
