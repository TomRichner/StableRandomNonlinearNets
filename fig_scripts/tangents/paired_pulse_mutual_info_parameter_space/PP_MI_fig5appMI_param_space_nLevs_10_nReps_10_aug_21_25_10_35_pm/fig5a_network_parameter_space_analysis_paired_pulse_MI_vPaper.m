...
% Set use_parameter_space = false to run only default parameters with multiple seeds
% Set use_parameter_space = true to run parameter grid search with multiple seeds per combination
use_parameter_space = true; % Change to false for default-only mode

%% Parameters for Grid Search (only used if use_parameter_space = true)
params_for_grid = {'EE_factor', 'mean_weight'};

%% Analysis Parameters
n_levels = 10; % Number of values to test for each parameter in the grid (if using parameter space)
n_repetitions = 10; % Number of repetitions (different seeds) per parameter combination
note = 'ppMI_param_space';

% Update note based on mode
if ~use_parameter_space
    note = [note '_default_only'];
    params_for_grid = {}; % Empty for default-only mode
    n_levels = 1; % Only one "level" (the default values)
end
...
if use_parameter_space
    output_dir_base = fullfile(pwd, ['PP_MI_fig5a' note '_nLevs_' num2str(n_levels) '_nReps_' num2str(n_repetitions) '_' dt_str]);
else
    output_dir_base = fullfile(pwd, ['PP_MI_fig5a' note '_nReps_' num2str(n_repetitions) '_' dt_str]);
end
...
copyfile(fullfile(fileparts(mfilename('fullpath')), 'SRNN_caller_wrapped_for_paired_pulse_MI.m'), output_dir_base);
...
fprintf('\n\n======================================================\n');
if use_parameter_space
    fprintf('===== Running Paired-Pulse MI Parameter Space Analysis =====\n');
else
    fprintf('===== Running Paired-Pulse MI Default Parameters Analysis =====\n');
end
fprintf('======================================================\n\n');
...
if use_parameter_space && ~isempty(params_for_grid)
    % Validate that all specified parameters for the grid exist in ranges
    all_possible_params = fieldnames(ranges);
    invalid_params = setdiff(params_for_grid, all_possible_params);
    if ~isempty(invalid_params)
    ...
    % Default-only mode: single configuration with default parameters
    param_vectors = {};
    num_combinations = 1;
    all_configs = {struct()}; % Empty struct means use all defaults
end

% Randomize the order of execution by shuffling the configs
shuffled_configs = all_configs(randperm(num_combinations));

fprintf('=== Paired-Pulse Parameter Space Exploration Started ===\n');
if use_parameter_space
    fprintf('Grid parameters: %s\n', strjoin(params_for_grid, ', '));
    fprintf('Levels per parameter: %d\n', n_levels);
else
    fprintf('Mode: Default parameters only\n');
end
fprintf('Repetitions per parameter combination: %d\n', n_repetitions);
fprintf('Parameter combinations: %d\n', num_combinations);
fprintf('Total simulations per condition: %d\n', num_combinations * n_repetitions);
fprintf('=========================================================\n\n');

%% Run Simulations in Batches, Interleaving Conditions and Repetitions
batch_size = 10; % Number of parameter configs per batch (before repetitions)
num_batches = ceil(num_combinations / batch_size);
num_conditions = length(conditions);
total_jobs = num_combinations * n_repetitions * num_conditions;
fprintf('Running %d parameter combinations × %d repetitions × %d conditions = %d total jobs in %d batches.\n', ...
    num_combinations, n_repetitions, num_conditions, total_jobs, num_batches);
...
for batch_idx = 1:num_batches
    ...
    num_jobs_in_batch = current_batch_size * n_repetitions * num_conditions;

    fprintf('--- Running Batch %d/%d (Configs %d-%d, %d reps each, %d conditions) ---\n', ...
        batch_idx, num_batches, start_idx, end_idx, n_repetitions, num_conditions);

    % --- Create a list of all jobs for the parfor loop ---
    jobs_to_run = cell(num_jobs_in_batch, 1);
    job_counter = 1;
    for config_k = 1:current_batch_size
        for rep_k = 1:n_repetitions
    ...
    % for j = 1:num_jobs_in_batch
        ...
        % Generate unique seed for this specific job
        base_seed = current_job.config_run_idx + (current_job.condition_idx-1)*num_combinations;
        sim_seed = base_seed + (current_job.repetition_idx-1)*num_combinations*num_conditions;
        ...
        % try
            ...
            result_to_save.repetition_idx = current_job.repetition_idx;
        ...
        %         'repetition_idx', current_job.repetition_idx);
        % 
        %     parallel_results{j} = result_to_save;
        % end
    end
    ...
    % Results are now 3D: [config_idx, repetition_idx, condition_idx]
    batch_results_by_condition = struct();
    for cond_k = 1:num_conditions
        batch_results_by_condition.(conditions{cond_k}.name) = cell(current_batch_size, n_repetitions);
    end

    for j = 1:num_jobs_in_batch
        condition_name = jobs_to_run{j}.condition.name;
        config_idx_in_batch = floor((j-1)/(n_repetitions*num_conditions)) + 1;
        rep_idx = mod(floor((j-1)/num_conditions), n_repetitions) + 1;
        batch_results_by_condition.(condition_name){config_idx_in_batch, rep_idx} = parallel_results{j};
    end
...
for c2_idx = 1:num_conditions
    ...
    matObj.(condition_name2).results = cell(num_combinations, n_repetitions);
end
    ...
    if exist(batch_save_filename, 'file')
        ...
        if isfield(loaded_data, 'batch_results_by_condition')
            for c2_idx = 1:num_conditions
                ...
                matObj.(condition_name2).results(start_idx:end_idx, :) = batch_slice;
            end
        else
